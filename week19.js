...
function makeTwo() {
  fetch("https://emojihub.yurace.pro/api/random/group/face-positive")
    ...
    .then((data) => {
      ...
      resultElement.innerHTML = data.htmlCode;
    })
    .catch((error) => console.error(error));
}
...
function makeFive() {
  fetch("https://www.boredapi.com/api/activity")
    .then((response) => response.json())
    .then((data) => console.log(`Number of participants: ${data.participants}`))
    .catch((error) => console.error(error));
}
...
function makeTen() {
  fetch("https://dog.ceo/api/breeds/image/random")
    ...
    .then((data) => {
      ...
      resultElement.insertAdjacentElement("afterend", imageElement);
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    });
...
function makeTwelve() {
  ...
  fetch(`http://ip-api.com/json/${ipAddress}`)
    .then((response) => response.json())
    .then((data) => {
      const geoPosition = `Страна: ${data.country}, Регион: ${data.regionName}, Город: ${data.city}, Широта: ${data.lat}, Долгота: ${data.lon}`;
      document.getElementById("result12").textContent = geoPosition;
    })
